...
describe('Searchbox', () => {
  ...
  const BASESEARCHLIMIT: number = parseInt(process.env.REACT_APP_DEFAULT_BULK_RETURN_LIMIT ?? ""); // ?? "0" only exists to prevent type errors. It should never be reached.
  ...
  test('opens filter menu when filter icon is clicked with default values', () => {
    ...
    expect(statusElement.firstElementChild?.textContent).toBe(ItemStatus.AVAILABLE);
    fireEvent.mouseDown(statusElement.firstElementChild!);
    Object.values(ItemStatus).forEach((value) => {
      const options = screen.getAllByRole('option');
      const filteredOptions = options.filter((option) => option.textContent === value.replace('_', ' '));
      expect(filteredOptions.length).toBe(1);
    });

    var searchTypeElement = screen.getByTestId('Search Type-simple-select')
    expect(searchTypeElement).toBeInTheDocument();
    expect(searchTypeElement.firstElementChild?.textContent).toBe(SearchType.LISTINGS);
  ...
  test('opens filter menu when filter icon is clicked with prior search values', () => {
    ...
    expect(screen.getByLabelText('Min Price')).toHaveValue(testSearchRequest.minPrice?.toString());
    expect((screen.getByLabelText('Max Price') as HTMLInputElement)).toHaveValue(testSearchRequest.maxPrice?.toString());
    expect(screen.getByTestId('Status-simple-select').firstElementChild?.textContent).toBe(testSearchRequest.status?.toString());
    expect(screen.getByTestId('Search Type-simple-select').firstElementChild?.textContent).toBe(testSearchRequest.searchType?.toString());
    expect((screen.getByLabelText('Latitude') as HTMLInputElement)).toHaveValue(testSearchRequest.latitude?.toString());
    expect((screen.getByLabelText('Longitude') as HTMLInputElement)).toHaveValue(testSearchRequest.longitude?.toString());
  });
  ...
  test('filter menu can be interacted with and calls expected methods', () => {
    ...
    fireEvent.change(minPriceInput, { target: { value: testSearchRequest.minPrice?.toString() } });
    expect(minPriceInput).toHaveValue(testSearchRequest.minPrice?.toString());

    const maxPriceInput = screen.getByLabelText('Max Price');
    fireEvent.change(maxPriceInput, { target: { value: testSearchRequest.maxPrice?.toString() } });
  ...
  test('clicking the search icon triggers the submit function', () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('Search...'), { target: { value: testSearchRequest.query } });
    fireEvent.change(screen.getByLabelText('Min Price'), { target: { value: testSearchRequest.minPrice?.toString() } });
    fireEvent.change(screen.getByLabelText('Max Price'), { target: { value: testSearchRequest.maxPrice?.toString() } });
    fireEvent.mouseDown(screen.getByTestId('Status-simple-select').firstElementChild!);
    fireEvent.click(screen.getByText(testSearchRequest.status!.toString()));
    fireEvent.mouseDown(screen.getByTestId('Search Type-simple-select').firstElementChild!);
    fireEvent.click(screen.getByText(testSearchRequest.searchType!.toString()));
    fireEvent.change(screen.getByLabelText('Latitude'), { target: { value: testSearchRequest.latitude?.toString() } });
    fireEvent.change(screen.getByLabelText('Longitude'), { target: { value: testSearchRequest.longitude?.toString() } });
