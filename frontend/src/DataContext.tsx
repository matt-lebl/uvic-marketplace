...
export function GetData(context: DataContextType | undefined, id: string): any {
  ...
  return (
    context.context.data.find((element) => element.id === id)?.dataObject ??
    null
  )
}
...
export function AddData(
  ...
  if (existingData !== undefined) {
    ...
    context.setContext({
      data: [...context.context.data, { id: id, dataObject: newData }],
    })
  }
}
...
export const RemoveData = (
  ...
  context.setContext({
    data: context.context.data.filter((element) => element.id !== id),
  })
  return context
}
